 ...
 * @v msb		MSB of expected result
 */
#define bigint_add_ok( addend, value, expected, msb ) do {		\
	...
	bigint_add ( &addend_temp, &value_temp, &carry );		\
	DBG_HDA ( 0, &value_temp, sizeof ( value_temp ) );		\
	...
	if ( shift ) {							\
		ok ( carry == 0 );					\
		ok ( ( value_temp.element[ size - 1 ] >> shift ) ==	\
		     msb );						\
	} else {							\
		ok ( carry == msb );					\
	}								\
 ...
 * @v msb		MSB of expected result
 */
#define bigint_subtract_ok( subtrahend, value, expected, msb ) do {	\
	...
	DBG_HDA ( 0, &value_temp, sizeof ( value_temp ) );		\
	bigint_subtract ( &subtrahend_temp, &value_temp, &carry );	\
	DBG_HDA ( 0, &value_temp, sizeof ( value_temp ) );		\
	bigint_done ( &value_temp, result_raw, sizeof ( result_raw ) );	\
									\
	ok ( memcmp ( result_raw, expected_raw,				\
