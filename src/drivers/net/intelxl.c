...
/**
 * Query VSI bandwidth configuration
 *
 * @v intelxl		Intel device
 * @ret rc		Return status code
 */
static int intelxl_admin_bandwidth ( struct intelxl_nic *intelxl ) {
	struct intelxl_admin_descriptor cmd;
	struct intelxl_admin_bandwidth_params *bandwidth =
		&cmd.params.bandwidth;
	struct intelxl_admin_bandwidth_buffer *buf =
		&intelxl->command.buffer->bandwidth;
	int rc;

	/* Populate descriptor */
	memset ( &cmd, 0, sizeof ( cmd ) );
	cmd.opcode = cpu_to_le16 ( INTELXL_ADMIN_BANDWIDTH );
	cmd.flags = cpu_to_le16 ( INTELXL_ADMIN_FL_BUF );
	cmd.len = cpu_to_le16 ( sizeof ( *buf ) );
	bandwidth->vsi = cpu_to_le16 ( intelxl->vsi );
	...
	DBGC ( intelxl, "INTELXL %p VSI %#04x qset %#04x\n",
	       intelxl, intelxl->vsi, intelxl->qset );
...
intelxl_context_dump ( struct intelxl_nic *intelxl, uint32_t op, size_t len ) {
	...
	for ( index = 0 ; ( sizeof ( line ) * index ) < len ; index++ ) {
		...
		pfcm_lanctxctl =
			( INTELXL_PFCM_LANCTXCTL_QUEUE_NUM ( intelxl->queue ) |
...
static int intelxl_context_line ( struct intelxl_nic *intelxl,
	...
	/* Start context operation */
	pfcm_lanctxctl = ( INTELXL_PFCM_LANCTXCTL_QUEUE_NUM ( intelxl->queue ) |
			   INTELXL_PFCM_LANCTXCTL_SUB_LINE ( index ) |
...
static int intelxl_probe ( struct pci_device *pci ) {
	...
	intelxl_init_ring ( &intelxl->tx, INTELXL_TX_NUM_DESC,
			    INTELXL_QTX ( 0 ), intelxl_context_tx );
	intelxl_init_ring ( &intelxl->rx, INTELXL_RX_NUM_DESC,
			    INTELXL_QRX ( 0 ), intelxl_context_rx );
	...
	intelxl->queue = INTELXL_PFLAN_QALLOC_FIRSTQ ( pflan_qalloc );
	DBGC ( intelxl, "INTELXL %p PF %d using port %d queue %d\n",
	       intelxl, intelxl->pf, intelxl->port, intelxl->queue );
...
static struct pci_device_id intelxl_nics[] = {
	...
	PCI_ROM ( 0x8086, 0x1584, "xl710-qsfp-b", "XL710 40GbE QSFP+", 0 ),
