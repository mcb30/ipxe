...
struct srp_login_req {
	...
	struct srp_port_id initiator_port_id;
	/** Target port identifier */
	struct srp_port_id target_port_id;
} __attribute__ (( packed ));
...
struct srp_device {
	...
	/** Send SRP information unit
	 *
	 * @v srp		SRP device
	 * @v iobuf		SRP information unit
	 * @ret rc		Return status code
	 */
	int ( * send ) ( struct srp_device *srp, struct io_buffer *iobuf );
	/** Receive SRP information unit
	 *
	 * @v srp		SRP device
	 * @v iobuf		SRP information unit
	 * @ret rc		Return status code
	 */
	int ( * recv ) ( struct srp_device *srp, struct io_buffer *iobuf );

	/** Backing device */
	struct refcnt *backend;
};

extern int srp_attach ( struct scsi_device *scsi );
