 ...
 * operation.
 */
static int pxe_udp_deliver ( struct udp_interface *udp,
			     struct io_buffer *iobuf,
			     struct sockaddr_tcpip *peer,
			     struct sockaddr_tcpip *local,
			     struct net_device *netdev __unused ) {
	struct pxe_udp_connection *pxe_udp = 
		container_of ( udp, struct pxe_udp_connection, udp );
...
static struct udp_interface_operations pxe_udp_operations = {
	.close		= ignore_udp_close,
	.deliver	= pxe_udp_deliver,
};

/** The PXE UDP connection */
static struct pxe_udp_connection pxe_udp = {
	.udp = UDP_INIT ( pxe_udp, &pxe_udp_operations ),
	.local = {
		.sin_family = AF_INET,
	},
};
...
PXENV_EXIT_t pxenv_udp_open ( struct s_PXENV_UDP_OPEN *pxenv_udp_open ) {
	...
	udp_close ( &pxe_udp.udp, 0 );
	if ( ( rc = udp_open_promisc ( &pxe_udp.udp ) ) != 0 ) {
...
PXENV_EXIT_t pxenv_udp_write ( struct s_PXENV_UDP_WRITE *pxenv_udp_write ) {
	...
	/* Transmit packet */
	if ( ( rc = udp_deliver ( &pxe_udp.udp, iobuf,
				  ( struct sockaddr_tcpip * ) &dest,
				  ( struct sockaddr_tcpip * ) &pxe_udp.local,
				  NULL ) ) != 0 ) {
